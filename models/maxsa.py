...
class MaxSA(nn.Module):
    def __init__(self,
        ...
        self.MaxSA = nn.ModuleList([
            MaxSA_Transformer(dim=dim,
                             n_head=n_head,
                             ffn_dim=ffn_dim,
                             is_shift = True)])
                            #  is_shift=True if iter_ % 2 == 1 else False) 
    ...
    def forward(self, 
        ...
        for MaxSA in self.MaxSA:
            feat_rt = MaxSA(feat_rt, feat_tr,
                           width=w, height=h,
        ...
        feat_f = torch.add(feat_r * self.weight, feat_t * (1 - self.weight))
...
class MaxSA_Transformer(nn.Module):
    ...
    def __init__(self,
                 ...
                 is_shift=True,
                 **kwargs):
        super(MaxSA_Transformer, self).__init__()
...
if __name__ == "__main__":
    ...
    input_rgb = torch.Tensor(1, 256, 20, 20)
    input_ir = torch.Tensor(1, 256, 20, 20)
    x=  [input_rgb, input_ir]

    MaxSA = MaxSA(n_iter=1,
                 dim=256,
                 n_head=1,
                 ffn_dim=4,
                 n_window=4)
