...
def main(img_vis_path, img_ir_path):
    ...
    if args.method == 'gradcam':
        for target_layer in args.target_layer:
            ...
            result = torch_img_ir.squeeze(0).mul(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).detach().cpu().numpy()
            result = result[..., ::-1]  # convert to bgr
            
            # 保存设置
            image_name = os.path.basename(img_ir_path)  # 获取图片名
